...
dem_ballots_by_dist_party <- ballots %>% filter(voter_party_code=='DEM') %>% group_by(cong_dist_desc,voter_party_code) %>% summarise(sum(abs_ballots,na.rm=TRUE))
ballots_by_dist <- ballots %>% group_by(cong_dist_desc)  %>% summarise(sum(abs_ballots,na.rm=TRUE))
dem_ballots_by_dist <- ballots %>% filter(voter_party_code=='DEM') %>% group_by(cong_dist_desc)  %>% summarise(sum(abs_ballots,na.rm=TRUE))
...
dem_votes_by_dist_party <- votes %>% filter(choice_party == 'DEM') %>% group_by(contest_name,choice_party) %>% summarise(sum(abs_votes,na.rm = TRUE))
votes_by_dist <- votes %>% group_by(contest_name) %>% summarise(sum(abs_votes,na.rm = TRUE))
dem_votes_by_dist <- votes %>% filter(choice_party == 'DEM') %>% group_by(contest_name) %>% summarise(sum(abs_votes,na.rm = TRUE))
