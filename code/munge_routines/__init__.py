...
def get_upsert_id(schema,table,req_var_d,other_var_ds,con,cur):
    ...
    q = 'WITH input_rows('+','.join(id_slots)+') AS (VALUES ('+var_slots+') ), ins AS (INSERT INTO {0}.{1} ('+','.join(id_slots)+') SELECT * FROM input_rows ON CONFLICT ({2}) DO NOTHING RETURNING id, '+','.join(id_slots)+') SELECT \'inserted\' AS source, id, '+','.join(id_slots)+' FROM ins UNION  ALL SELECT \'selected\' AS source, c.id, '+  ','.join(['c.'+i for i in id_slots])  +' FROM input_rows JOIN {0}.{1} AS c USING ('+ ','.join(id_slots)+');'
