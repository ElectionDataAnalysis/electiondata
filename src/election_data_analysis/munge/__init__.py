...
def get_and_check_munger_params(munger_path: str) -> (dict, Optional[dict]):
    ...
    # TODO check that each lookup section has a replacement formula for each element referencing the lookup field
    ...
    for h in headers:
        try:
            ...
            continue
        relevant_elements = [e for e in formulas.keys() if f"<{lookup_field}>" in formulas[e]]
        re_formulas, new_err = ui.get_parameters(
                required_keys=[f"{e}_replacement" for e in relevant_elements],
                param_file=munger_path,
                header=h,
            )
...
def to_standard_count_frame(
    ...
    for sheet in keys_in_order:
        ...
        non_fatal_dfs = [n for n in range(len(df_list)) if not ui.fatal_error(error_by_df[n])]
        if non_fatal_dfs:
        ...
        error_by_sheet[sheet] = ui.consolidate_errors([error_by_df[n] for n in range(len(df_list))])

    # if even one sheet lacks a fatal error, consider all errors non-fatal for this file
    non_fatal_sheets = [k for k in raw_dict.keys() if not ui.fatal_error(error_by_sheet[k])]
    if non_fatal_sheets:
