...
@pytest.mark.skipif(not ok["az20p"], reason="No AZ 2020 Primary data")
def test_az_state_senate_totals():
    assert (
        check_contest_totals(
            "2020 Primary",
            "Arizona",
            "AZ Senate District 17 (Republican Party)",
        )
        == 4896
    )

@pytest.mark.skipif(not ok["az20p"], reason="No AZ 2020 Primary data")
def test_az_state_rep_totals():
    assert (
        check_contest_totals(
            "2020 Primary",
            "Arizona",
            "US House AZ District 4",
        )
        == 1977
    )

@pytest.mark.skipif(not ok["az20p"], reason="No AZ 2020 Primary data")
def test_az_contest_by_vote_type():
    assert True == True

@pytest.mark.skipif(not ok["az20p"], reason="No AZ 2020 Primary data")
def test_az_totals_match_vote_type():
    assert True == True
