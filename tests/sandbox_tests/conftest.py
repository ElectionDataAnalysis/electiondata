...
def pytest_generate_tests(metafunc):
    ...
    if "election" in metafunc.fixturenames and option_value is not None:
        metafunc.parametrize("election", [option_value])

    option_value = metafunc.config.option.param_file
    if "jurisdiction" in metafunc.fixturenames and option_value is not None:
        metafunc.parametrize("jurisdiction", [option_value])
