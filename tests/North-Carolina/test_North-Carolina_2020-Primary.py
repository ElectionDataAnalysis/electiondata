...
def test_nc_contest_by_vote_type_20(dbname):
    assert (
        not e.data_exists("2020 Primary", "North Carolina", dbname=dbname)
...
def test_nc_totals_match_vote_type_20(dbname):
    assert (
        not e.data_exists("2020 General", "North Carolina", dbname=dbname)
        or e.check_totals_match_vote_types(
            "2020 General", "North Carolina", dbname=dbname
        )
        == True
    )
