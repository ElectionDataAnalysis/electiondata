...
def test_loading(dataloader, test_data_url, param_file):
    ...
    assert attempted_pairs, "No dataloading attempted; check run_time.ini or specified parameter file"

    # all files loaded successfully
    assert all([v == list() for v in failed_to_load.values()]), "Not all files loaded successfully"

    # all tests passed
    assert all([v for v in all_tests_passed.values()]), "Some tests failed"
